<template>
  <div class="codeLogin-template">
    <!-- <headers isBack></headers> -->
    <span class="title">手机验证码登录</span>
    <ul class='ul'>
      <li class="account" :class="isfocu == 0 && 'active'">
        <div><i></i></div>
        <input type="text" placeholder-style="color: #BBBBBB;font-weight: 400" placeholder="请输入手机号"v-model="name"
        @focus='focu(0)' @blur="blur">
      </li>
      <li class="code" :class="isfocu == 1 && 'active'">
        <div><i></i></div>
        <input type="text" placeholder-style="color: #BBBBBB;font-weight: 400" placeholder="请输入验证码" v-model="code"
        @focus='focu(1)' @blur="blur">
    ...
    <div class="btn">
      <button type="button" :disabled=" !name || !code " >登录</button>
    </div>
  </div>
</template>
